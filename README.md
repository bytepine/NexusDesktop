...
| 接入方式 | 端点 | 适用 |
|----------|------|------|
| **NexusDesktop**（本程序） | `http://127.0.0.1:6900/stream` | 无需 IDE 插件；任意 AI 客户端；双击启动 |
...
| ✓ 启用中转服务器 | 启停 MCP HTTP 监听（默认 `:6900`） |
...
```
AI 客户端 ──POST /stream──► MCP HTTP Server (:6900)
                                    │
...
4. Point your AI client to `http://127.0.0.1:6900/stream`
